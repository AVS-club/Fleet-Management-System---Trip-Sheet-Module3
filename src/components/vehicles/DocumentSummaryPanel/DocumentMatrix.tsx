...
interface DocumentMatrixProps {
  ...
  onCheckChallan?: (vehicleId: string) => void;
  refreshingVehicles?: Set<string>;
  challanLoadingVehicles?: Set<string>;
}
            ...
            <div className="flex items-center gap-1">
              ...
              {onRefreshVehicle && (
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    onRefreshVehicle(vehicle.id);
                  }}
                  disabled={refreshingVehicles.has(vehicle.id)}
                  className="p-1 hover:bg-blue-100 bg-blue-50 rounded transition-all disabled:opacity-50 border border-blue-200"
                  title="Refresh this vehicle's documents"
                >
                  <RefreshCw className={`h-3.5 w-3.5 text-blue-600 ${refreshingVehicles.has(vehicle.id) ? 'animate-spin' : ''}`} />
                </button>
              )}
              ...
              {onCheckChallan && (
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    onCheckChallan(vehicle.id);
                  }}
                  disabled={challanLoadingVehicles.has(vehicle.id)}
                  className="p-1 hover:bg-yellow-100 bg-yellow-50 rounded transition-all disabled:opacity-50 border border-yellow-200"
                  title="Check challans for this vehicle"
                >
                  <AlertTriangle className={`h-3.5 w-3.5 text-yellow-600 ${challanLoadingVehicles.has(vehicle.id) ? 'animate-spin' : ''}`} />
                </button>
              )}
            </div>
          </div>
