...
const NextServiceReminderSection: React.FC<Props> = ({ reminder, onToggle, odometerReading }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-5 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Switch checked={reminder} onChange={(e) => onToggle(e.target.checked)} label="Set Reminder" size="sm" />
      </div>
      {reminder && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <Input label="Next Service Date" type="date" icon={<Calendar className="h-4 w-4" />} {...register('next_service_due.date')} />
          <Input
