...
const PartsAnalyticsPanel: React.FC<PartsAnalyticsPanelProps> = ({
        ...
        if (Array.isArray(task.service_groups)) {
          task.service_groups.forEach((group) => {
            ...
            // For batteries
              ...
              partTypeFilter === "batteries" &&
              group.battery_tracking &&
              group.battery_brand
            ) {
  ...
  const chartData = useMemo(() => {
    ...
    const costPerBrand = Object.entries(brandCosts)
      .map(([brand, data]) => ({
        brand,
    ...
    const countPerBrand = Object.entries(brandCosts)
      .map(([brand, data]) => ({
        brand,
        count: data.count,
      }))
  ...
  const hasData =
    chartData.costPerBrand.length > 0 && chartData.countPerBrand.length > 0;
        ...
        {hasData ? (
          ...
          <div className="bg-gray-50 rounded-lg p-6 text-center">
            <p className="text-gray-500">
              No data available for this part type or brand selection.
            </p>
          </div>
        )}
