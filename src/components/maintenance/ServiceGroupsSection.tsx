...
const ServiceGroupsSection: React.FC = () => {
  ...
  const { fields, append, remove } = useFieldArray({ control, name: 'service_groups' });
  const serviceGroupsWatch = watch('service_groups');

  const totalCost = serviceGroupsWatch?.reduce((sum, group) => sum + (parseFloat((group as any).cost) || 0), 0) || 0;
    ...
    <div className="bg-white rounded-lg shadow-sm p-5 space-y-4">
      ...
      <div className="space-y-4">
        ...
        <Button
          ...
          size="sm"
          onClick={() =>
            append({ vendor_id: '', tasks: [], cost: 0, battery_tracking: false, tyre_tracking: false })
          }
        ...
        {fields.length > 0 && (
          <div className="p-4 bg-success-50 rounded-lg border border-success-200">
            <div className="flex justify-between items-center">
              ...
              <p className="text-lg font-semibold text-success-700">₹{totalCost.toLocaleString()}</p>
            </div>
          </div>
        )}
      </div>
