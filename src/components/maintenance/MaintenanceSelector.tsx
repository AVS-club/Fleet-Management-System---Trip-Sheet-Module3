...
const MaintenanceSelector: React.FC<MaintenanceSelectorProps> = ({
  ...
  const { items: maintenanceItems, groups: maintenanceGroups } =
    React.useMemo(() => {
      if (!catalogData) {
        return { items: [], groups: {} };
      }
          ...
          >
            <div className="p-2 border-b sticky top-0 bg-white z-10">
              <input
                ref={searchInputRef}
                type="text"
                className="w-full p-2 border bg-white z-10 rounded-md"
                  ...
                  Object.entries(maintenanceGroups).map(([groupKey, group]) => {
                    ...
                    const filteredGroupItems = group.items.filter(
                      (item) =>
                        item.name
                          .toLowerCase()
                          .includes(searchTerm.toLowerCase()) && !item.inactive
                    );
                    ...
                    return (
                      <div key={groupKey} className="border-b last:border-b-0">
                        <div className="p-2 bg-gray-50 font-medium text-sm text-gray-700 top-[41px] z-10">
                          {group.title}
                        </div>
