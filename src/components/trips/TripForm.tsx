...
const TripForm: React.FC<TripFormProps> = ({
  ...
  // Update P&L fields
  useEffect(() => {
    setValue('income_amount', liveCalculations.income);
    setValue('total_expense', liveCalculations.totalExpense);
    setValue('net_profit', liveCalculations.profit);
    setValue('cost_per_km', liveCalculations.costPerKm);
    setValue('profit_status', liveCalculations.profit > 0 ? 'profit' : liveCalculations.profit < 0 ? 'loss' : 'neutral');
  }, [liveCalculations, setValue]);

  // Handle route analysis
  const handleAnalyzeRoute = async () => {
    if (!selectedWarehouseId || selectedDestinationObjects.length === 0) {
      toast.warning('Please select origin and at least one destination');
      return;
    }
        ...
        {showCalculations && (
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
            ...
            <div className="bg-white rounded-lg p-2 shadow-sm">
              ...
              <div className="text-lg font-bold text-blue-600">
                ₹{liveCalculations.income}
              </div>
            </div>
            
            <div className="bg-white rounded-lg p-2 shadow-sm">
              <div className="text-xs text-gray-500 flex items-center">
                {liveCalculations.profit >= 0 ? (
                  <TrendingUp className="h-3 w-3 mr-1" />
                ) : (
              ...
              <div className={`text-lg font-bold flex items-center ${
                liveCalculations.profit >= 0 ? 'text-green-600' : 'text-red-600'
              }`}>
                {liveCalculations.profit >= 0 ? '+' : '-'}₹{Math.abs(liveCalculations.profit)}
              </div>
              {liveCalculations.income > 0 && (
                <div className="text-xs text-gray-500">
                  {liveCalculations.profitMargin}%
                </div>
              )}
            </div>
          </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 border-l-4 border-purple-500">
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            disabled={isAnalyzing || !selectedWarehouseId || selectedDestinationObjects.length === 0}
            className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center gap-2"
          >
            {isAnalyzing ? (
              <>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 border-l-4 border-indigo-500">
        <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-3 flex items-center">
          ...
          <span className={`ml-auto text-sm font-bold ${
            ...
            {liveCalculations.profit >= 0 ? 'Profit' : 'Loss'}: ₹{Math.abs(liveCalculations.profit)}
          </span>
        </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <Controller
              ...
              render={({ field }) => (
                <select
                  {...field}
                  className="w-full px-3 py-2 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400"
                >
          ...
          <div>
            <Input
              ...
              step="0.01"
              icon={<IndianRupee className="h-4 w-4" />}
              {...register('freight_rate', { valueAsNumber: true })}
            />
          </div>
          
          {watchedValues.billing_type === 'per_ton' && (
            <div>
              <Input
          ...
          {watchedValues.billing_type === 'manual' && (
            <div>
              <Input
                label="Income Amount"
                type="number"
        ...
        <div className="mt-4 p-3 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-lg">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
            <div>
              ...
              <span className="ml-2 font-bold text-blue-600">₹{liveCalculations.income}</span>
            </div>
            <div>
              <span className="text-gray-500">Expenses:</span>
              <span className="ml-2 font-bold text-red-600">₹{liveCalculations.totalExpense}</span>
            </div>
            <div>
              <span className="text-gray-500">Net Profit:</span>
              <span className={`ml-2 font-bold ${liveCalculations.profit >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                ₹{liveCalculations.profit}
              </span>
            </div>
            <div>
              <span className="text-gray-500">Margin:</span>
              <span className={`ml-2 font-bold ${liveCalculations.profitMargin >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                {liveCalculations.profitMargin}%
              </span>
            </div>
          </div>
