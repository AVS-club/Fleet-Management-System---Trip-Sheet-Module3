...
const RouteAnalytics: React.FC<RouteAnalyticsProps> = ({ 
  ...
  const getRouteIcon = (index: number) => {
    switch (index) {
      case 0:
        return { color: 'bg-yellow-100 text-yellow-600', icon: '🏆' };
      case 1:
        return { color: 'bg-gray-100 text-gray-600', icon: '🥈' };
      case 2:
        return { color: 'bg-orange-100 text-orange-600', icon: '🥉' };
      default:
        return { color: 'bg-blue-100 text-blue-600', icon: `${index + 1}` };
    }
  };
    ...
    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
      ...
      {routes.length > 0 ? (
            ...
            return (
              <div key={index} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                ...
                <div className="text-right ml-4">
                  <p className="font-medium text-gray-900">{formatCurrency(route.avgCost)}</p>
        ...
        <div className="text-center py-8">
          ...
          <p className="text-sm text-gray-400 mt-1">Route analytics will appear here once trips are recorded</p>
        </div>
      )}
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-yellow-800">Most Frequent</span>
            </div>
            <p className="text-sm text-yellow-600 mt-1 truncate">
          ...
          <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-green-800">Unique Routes</span>
            </div>
