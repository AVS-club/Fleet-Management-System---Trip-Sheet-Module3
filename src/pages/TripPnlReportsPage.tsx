...
const TripPnlReportsPage: React.FC = () => {
  ...
  const pnlSummary = useMemo((): TripSummaryMetrics => {
    const summary = filteredTrips.reduce((acc, trip) => {
      ...
      if (trip.fuel_quantity && !trip.short_trip) {
        acc.totalFuel += trip.fuel_quantity;
      }
    ...
    const avgDistance = summary.totalTrips > 0 ? summary.totalDistance / summary.totalTrips : 0;
    const meanMileage = summary.totalFuel > 0 ? summary.totalDistance / summary.totalFuel : 0;
    ...
    filteredTrips.forEach(trip => {
      if (trip.driver_id) {
        ...
        driverStats[trip.driver_id].tripCount++;
        driverStats[trip.driver_id].totalDistance += ((trip.end_km || 0) - (trip.start_km || 0));
      }
    });
    ...
    const result: TripSummaryMetrics = {
      totalExpenses: summary.totalExpense,
      avgDistance,
      tripCount: summary.totalTrips,
      meanMileage,
      topDriver: topDriver ? {
        id: topDriver[0],
    ...
    console.log('P&L Summary:', result);
    return result;
  ...
  const exportToExcel = () => {
    const exportData = filteredTrips.map(trip => {
      ...
      return {
        ...
        'Date': format(new Date(trip.trip_start_date), 'dd/MM/yyyy'),
    ...
    return (
      <Layout title="Trip P&L Report" subtitle="Analyze profitability of trips">
        <div className="flex items-center justify-center h-64">
          ...
          <p className="ml-3 text-gray-600">Loading trip data...</p>
        </div>
      </Layout>
    );
  }
