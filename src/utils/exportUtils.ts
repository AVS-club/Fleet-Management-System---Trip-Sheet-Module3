...
export const generateVehiclePDF = async (
  ...
  doc.setFontSize(12);
  doc.setFont("helvetica", "normal");
  doc.text(`${vehicle.make} ${vehicle.model} (${vehicle.year})`, 105, 38, {
    align: "center",
  });
  ...
  if (vehicle.insurance_end_date) {
    const insuranceStatus =
      new Date(vehicle.insurance_end_date) > new Date() ? "Valid" : "Expired";
    y = addStatusBadge(doc, "Insurance", insuranceStatus, y);
    y = addField(
      doc,
      "Insurance Expiry",
      new Date(vehicle.insurance_end_date).toLocaleDateString(),
      y
    );
  } else {
  ...
  if (vehicle.fitness_expiry_date) {
    const fitnessStatus =
      new Date(vehicle.fitness_expiry_date) > new Date() ? "Valid" : "Expired";
    y = addStatusBadge(doc, "Fitness Certificate", fitnessStatus, y);
    y = addField(
      doc,
      "Fitness Expiry",
      new Date(vehicle.fitness_expiry_date).toLocaleDateString(),
      y
    );
  } else {
  ...
  if (vehicle.permit_expiry_date) {
    const permitStatus =
      new Date(vehicle.permit_expiry_date) > new Date() ? "Valid" : "Expired";
    y = addStatusBadge(doc, "Permit", permitStatus, y);
    y = addField(
      doc,
      "Permit Expiry",
      new Date(vehicle.permit_expiry_date).toLocaleDateString(),
      y
    );
  } else {
  ...
  if (vehicle.puc_expiry_date) {
    const pucStatus =
      new Date(vehicle.puc_expiry_date) > new Date() ? "Valid" : "Expired";
    y = addStatusBadge(doc, "PUC Certificate", pucStatus, y);
    y = addField(
      doc,
      "PUC Expiry",
      new Date(vehicle.puc_expiry_date).toLocaleDateString(),
      y
    );
  } else {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setTextColor(100, 100, 100);
    doc.text(
      `Auto Vital Solution - Vehicle Profile - Page ${i} of ${pageCount}`,
      105,
      287,
      { align: "center" }
    );
...
export const generateDriverPDF = async (
  ...
  if (driver.license_expiry_date) {
    const licenseStatus =
      new Date(driver.license_expiry_date) > new Date() ? "Valid" : "Expired";
  ...
  y = addField(
    doc,
    "Documents Verified",
    driver.documents_verified ? "Yes" : "No",
    y
  );
  ...
  if (primaryVehicle) {
    ...
    y = addField(
      doc,
      "Type",
      primaryVehicle.type.charAt(0).toUpperCase() +
        primaryVehicle.type.slice(1),
      y
    );
  ...
  const tripsWithMileage = trips.filter(
    (trip) => trip.calculated_kmpl && !trip.short_trip
  );
  const averageMileage =
    tripsWithMileage.length > 0
      ? tripsWithMileage.reduce(
          (sum, trip) => sum + (trip.calculated_kmpl || 0),
          0
        ) / tripsWithMileage.length
      : 0;
  ...
  y = addField(
    doc,
    "Average Mileage",
    averageMileage > 0 ? `${averageMileage.toFixed(2)} km/L` : "N/A",
    y
  );
  ...
  if (driver.performance_metrics?.risk_score !== undefined) {
    ...
    else if (driver.performance_metrics.risk_score > 30)
      riskLabel = "Medium Risk";

    y = addField(
      doc,
      "Risk Score",
      `${driver.performance_metrics.risk_score}% (${riskLabel})`,
      y
    );
  }
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setTextColor(100, 100, 100);
    doc.text(
      `Auto Vital Solution - Driver Profile - Page ${i} of ${pageCount}`,
      105,
      287,
      { align: "center" }
    );
...
export const downloadVehicleDocuments = async (
  vehicle: Vehicle
): Promise<void> => {
  try {
    const zip = new JSZip();
    ...
    if (vehicle.rc_document_url) {
      downloadPromises.push(
        addDocumentToZip(
          ...
          `RC_${vehicle.registration_number}.pdf`
        )
      );
    }
    ...
    if (vehicle.insurance_document_url) {
      downloadPromises.push(
        addDocumentToZip(
          ...
          `Insurance_${vehicle.registration_number}.pdf`
        )
      );
    }
    ...
    if (vehicle.fitness_document_url) {
      downloadPromises.push(
        addDocumentToZip(
          ...
          `Fitness_${vehicle.registration_number}.pdf`
        )
      );
    }
    ...
    if (vehicle.permit_document_url) {
      downloadPromises.push(
        addDocumentToZip(
          ...
          `Permit_${vehicle.registration_number}.pdf`
        )
      );
    }
    ...
    if (vehicle.puc_document_url) {
      downloadPromises.push(
        addDocumentToZip(
          ...
          `PUC_${vehicle.registration_number}.pdf`
        )
      );
    }
...
export const createShareableVehicleLink = async (
  vehicleId: string
): Promise<string> => {
  try {
...
export const createShareableDriverLink = async (
  driverId: string
): Promise<string> => {
  try {
