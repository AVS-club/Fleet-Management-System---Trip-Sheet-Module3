...
function parseDocumentAIResponse(response: any): InsuranceDetails {
  try {
    ...
    const result: InsuranceDetails = {
      ...
      vehicleNumber: findField(fieldMap, [
        "vehicle number",
        "registration number",
      ]),
...
export function extractInsuranceDetails(text: string): InsuranceDetails[] {
  ...
  for (const chunk of chunks) {
    ...
    const result: InsuranceDetails = {
      ...
      insurerName: extract(
        /insurer[:\s]+([a-z0-9\s&\-]+(?:insurance|assurance|limited))/i
      )?.value,
      validFrom: extract(/valid(?:\.|from)[:\s]+([0-9\/\-]{8,10})/i)?.value,
      validUntil: extract(/valid(?:\.|until|upto)[:\s]+([0-9\/\-]{8,10})/i)
        ?.value,
      vehicleNumber: extract(
        /vehicle(?:\.|registration|number)[:\s]*((?:cg|od)[0-9]{2}[a-z]{2}[0-9]{4})/i
      )?.value,
      coverage: extract(/coverage[:\s]+([a-z0-9\s\-,]+)/i)?.value,
      premium: parseFloat(
        extract(/premium[:\s]*(?:rs\.?|₹)?([0-9,]+)/i)?.value?.replace(
          /,/g,
          ""
        ) || "0"
      ),
