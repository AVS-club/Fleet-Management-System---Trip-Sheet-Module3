/*
  # Update Drivers Schema
  
  1. Changes
    - Rename 'experience' column to 'experience_years' for clarity
    - Add document URL columns (driver_photo_url, license_doc_url, etc.)
    - Create foreign key relationship between drivers and vehicles
    - Add triggers for data formatting (uppercase license, lowercase email)
    - Add appropriate indexes and constraints
    - Define the trigger_set_timestamp function if it doesn't exist
*/
...
-- Check if experience column exists and rename to experience_years if it does
DO $$
BEGIN
  IF EXISTS (
...
BEGIN
  IF NOT EXISTS (
    ...
    CHECK (status = ANY (ARRAY['active'::text, 'inactive'::text, 'onLeave'::text, 'suspended'::text, 'blacklisted'::text]));
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger
    WHERE tgname = 'set_timestamp_drivers'
  ) THEN
