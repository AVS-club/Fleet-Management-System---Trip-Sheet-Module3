...
-- Create policies without checking for existence first
-- Instead, we'll drop them first if they exist and then recreate them
DROP POLICY IF EXISTS "Enable read access for authenticated users" ON maintenance_tasks_catalog;
DROP POLICY IF EXISTS "Enable insert for authenticated users" ON maintenance_tasks_catalog;
DROP POLICY IF EXISTS "Enable update for authenticated users" ON maintenance_tasks_catalog;
