-- Fix division by zero error when fuel_quantity is 0
-- This happens when editing a trip and changing refueling status
...
BEGIN
    ...
    IF NEW.refueling_done AND NEW.fuel_quantity IS NOT NULL AND NEW.fuel_quantity > 0 THEN
        IF distance_km IS NOT NULL AND distance_km > 0 THEN
            -- Safe to divide now - we know fuel_quantity > 0
...
'Transport business friendly validation. Prevents division by zero errors and uses string concatenation for formatting.';
