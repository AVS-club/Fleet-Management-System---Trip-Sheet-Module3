-- Fix the format() error with currency symbols
-- Replace ₹ with Rs. to avoid PostgreSQL format() issues
...
BEGIN
    ...
    -- FIXED: Removed rupee symbol to avoid format() error
    IF NEW.fuel_expense IS NOT NULL THEN
        IF NEW.fuel_expense > 200000 THEN
            validation_warnings := array_append(validation_warnings,
                format('Very high fuel expense: Rs. %.2f - verify amount', NEW.fuel_expense));
        ELSIF NEW.fuel_expense > 50000 THEN
            validation_warnings := array_append(validation_warnings,
                format('High fuel expense: Rs. %.2f - verify amount', NEW.fuel_expense));
        END IF;
        ...
        IF NEW.fuel_quantity IS NOT NULL AND NEW.fuel_quantity > 0 THEN
            ...
            BEGIN
                IF price_per_liter > 300 THEN
                    validation_warnings := array_append(validation_warnings,
                        format('Very high fuel price: Rs. %.2f per liter - verify amount', price_per_liter));
                ELSIF price_per_liter < 50 THEN
                    validation_warnings := array_append(validation_warnings,
                        format('Unusually low fuel price: Rs. %.2f per liter', price_per_liter));
                END IF;
            END;
        END IF;
    END IF;
    ...
    IF NEW.total_road_expenses IS NOT NULL THEN
        IF NEW.total_road_expenses > 200000 THEN
            validation_warnings := array_append(validation_warnings,
                format('Very high total expenses: Rs. %.2f - verify breakdown', NEW.total_road_expenses));
        ELSIF NEW.total_road_expenses > 50000 THEN
            validation_warnings := array_append(validation_warnings,
                format('High total expenses: Rs. %.2f - verify breakdown', NEW.total_road_expenses));
        END IF;
    END IF;
...
'Validates trip value ranges with relaxed limits suitable for transport business. Removed 2000km distance limit and 30km/L mileage limit. Fixed format() errors with currency symbols.';
