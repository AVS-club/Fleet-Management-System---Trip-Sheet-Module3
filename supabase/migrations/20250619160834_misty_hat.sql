/*
  # Maintenance Tasks Catalog Migration
  
  This migration creates the maintenance_tasks_catalog table and populates it with
  predefined maintenance categories and tasks. It includes error handling to prevent
  issues with existing objects.
*/
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'Enable read access for authenticated users' AND tablename = 'maintenance_tasks_catalog'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'Enable insert for authenticated users' AND tablename = 'maintenance_tasks_catalog'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'Enable update for authenticated users' AND tablename = 'maintenance_tasks_catalog'
  ) THEN
