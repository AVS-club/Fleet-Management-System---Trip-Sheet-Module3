-- ================================================================
-- AUTO-REFRESH KPIs EVERY 30 MINUTES
-- ================================================================
-- This migration:
-- 1. Creates a stored procedure for KPI generation
-- 2. Enables pg_cron extension for scheduling
-- 3. Schedules automatic KPI refresh every 30 minutes
-- ================================================================

-- Enable pg_cron extension (requires superuser privileges)
-- Note: If this fails on Supabase, pg_cron is already enabled by default
CREATE EXTENSION IF NOT EXISTS pg_cron;

-- ================================================================
-- STEP 1: Create the KPI generation function
-- ================================================================

CREATE OR REPLACE FUNCTION public.generate_kpi_cards()
RETURNS TABLE(cards_created INTEGER)
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
...
BEGIN
  ...
  FOR org_record IN SELECT id, name FROM public.organizations ORDER BY created_at LOOP
    ...
    RAISE NOTICE '   ✅ Generated 8 KPI cards for %', org_record.name;
  ...
  -- Return the count
  RETURN QUERY SELECT total_kpi_count;
END;
$$;
...
-- ================================================================
-- STEP 2: Schedule automatic refresh every 30 minutes using pg_cron
-- ================================================================

-- Remove any existing cron job with the same name
SELECT cron.unschedule('refresh-kpis-every-30-minutes') WHERE EXISTS (
  SELECT 1 FROM cron.job WHERE jobname = 'refresh-kpis-every-30-minutes'
);

-- Schedule the KPI refresh to run every 30 minutes
-- This runs at :00 and :30 of every hour
SELECT cron.schedule(
  'refresh-kpis-every-30-minutes',  -- Job name
  '0,30 * * * *',                   -- Cron expression: every 30 minutes (at :00 and :30)
  $$SELECT public.generate_kpi_cards();$$ -- SQL command to execute
);

-- ================================================================
-- ALTERNATIVE: For 5-minute intervals (commented out)
-- ================================================================
-- If you want to refresh every 5 minutes instead, uncomment below:
--
-- SELECT cron.unschedule('refresh-kpis-every-30-minutes') WHERE EXISTS (
--   SELECT 1 FROM cron.job WHERE jobname = 'refresh-kpis-every-30-minutes'
-- );
...
-- ================================================================
-- Manual refresh command (for testing)
-- ================================================================
-- To manually refresh KPIs, run:
-- SELECT public.generate_kpi_cards();
