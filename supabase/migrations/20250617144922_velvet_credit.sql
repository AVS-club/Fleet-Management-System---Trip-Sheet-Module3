/*
  # Add Trip Summary RPC Function
  
  1. New Functions
    - `get_trip_summary_metrics` - Calculate summary metrics for trips with filtering
  
  2. Functionality
    - Calculates total expenses across all expense types
    - Determines average trip distance
    - Calculates mean mileage (excluding null values and short trips)
    - Identifies top driver by total distance covered
    - Identifies top vehicle by number of trips
    - All calculations respect filters for date range, vehicle, driver, warehouse, and trip type
  
  3. Parameters
    - `start_date` - Starting date for filtering trips
    - `end_date` - Ending date for filtering trips
    - `p_vehicle_id` - Filter by specific vehicle
    - `p_driver_id` - Filter by specific driver
    - `p_warehouse_id` - Filter by specific warehouse
    - `p_trip_type` - Filter by trip type (local, one_way, two_way)
*/
